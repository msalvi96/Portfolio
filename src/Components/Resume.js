...
class Resume extends Component {
  render() {
    ...
    if (this.props.portfolio) {
        var projects = this.props.portfolio.projects.map(function(project) {
        return <div key={project.title}>
          <a href={project.url}><h3>{project.title}</h3></a>
          <p className="info">{project.category}<span>&bull;</span> <em className="date">{project.duration}</em></p>
